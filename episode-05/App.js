    ...
    return (
        ...
        <div className="card-container">
            <div className="card-img">
                <img src={"https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/"+resData.data.cloudinaryImageId} alt=""/>
            </div>
            <div className="card-content">
              <h1>{resData.data.name}</h1>
                <small>{resData.data.cuisines.join(', ')}</small>
                <p>{resData.data.avgRating} Rating</p>
                <p>Deliver - {resData.data.deliveryTime} min</p>
                <p>Rs.{resData.data.costForTwo/100} For Two</p>
            </div>
        </div>
...
//not use index as key in loop like map reduce .. use unique id from api its recommended
...
//Unique key via api is > then index in loop( least priority)
                    resObj.map(res => <ResturantCard key={res.data.id} resData={res}/>)
                }
            </div>
        </div>
